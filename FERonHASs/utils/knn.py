...
if __name__ == '__main__':
    ...
    my_features = read_probs(feat_path,
                             980,
                             468,
                             np.float32,
                             )

    my_features = l2norm(my_features)
    my_index = knn_brute_force(my_features, k=80)
    my_knn_path = './data/knns/part1_test/faiss_k_80.npz'
    dump_data(my_knn_path, my_index.knns, force=True)
    my_knns = load_data(my_knn_path)
    print(len(my_knns))
